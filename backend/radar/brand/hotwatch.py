...
from .models import BrandMention, BrandMentionSnapshot
...
def rescore_mention(session: Session, mention: BrandMention) -> None:
    snaps = [
...
def hotwatch_tick(
    ...
    """Re-poll every hot BrandMention and rescore it.
    ...
    """
    q = session.query(BrandMention).filter_by(is_hot=True)
    if brand_ids is not None:
        if not brand_ids:
            return 0
        q = q.filter(BrandMention.brand_id.in_(list(brand_ids)))
    hot = q.all()
    for mention in hot:
        try:
            ...
            log.exception("Hot-watch failed for brand mention %s", mention.id)
    session.commit()
    return len(hot)
