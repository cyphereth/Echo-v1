"""Brand-domain post-collect processing: classify mentions + draft replies.

Rebound from radar/pipeline.py to Brand/BrandMention/BrandDraftEdit/BrandComment.
Legacy radar/pipeline.py is left byte-for-byte intact.
"""
...
from .models import Brand, BrandMention, BrandDraftEdit, BrandComment
...
def opportunity_for(m: BrandMention) -> Optional[str]:
...
def recent_edits(session: Session, brand_id: int) -> list[dict]:
    ...
    rows = (
        session.query(BrandDraftEdit)
        .filter_by(brand_id=brand_id)
        .order_by(BrandDraftEdit.created_at.desc())
        .limit(5)
        .all()
    )
...
def classify_and_draft(session: Session, brand_id: int) -> dict:
    """Classify every unclassified brand mention for a brand, then draft the top-N by severity."""
    ...
    unclassified = (
        ...
        .filter(BrandMention.brand_id == brand_id, BrandMention.category.is_(None),
                BrandMention.is_spam.is_(False))
        .all()
    )
...
def fetch_and_store_comments(session: Session, mention: BrandMention,
...
def fetch_new_comments(session: Session, brand_id: int, provider, tg_provider) -> int:
    ...
    brand mentions that don't have comments yet."""
    candidates = (
        session.query(BrandMention)
        .filter(BrandMention.brand_id == brand_id,
                BrandMention.is_spam.is_(False),
                BrandMention.source.in_(("competitor", "niche")),
                # Chat messages (composite post_id "ns/msgid") are standalone — they
                # have no channel-style reply thread, so don't even select them.
                ~BrandMention.post_id.like("%/%"),
                ~BrandMention.comment_rows.any())
        .order_by(BrandMention.first_seen.desc())
    ...
    for m in candidates:
        try:
            ...
            log.exception("Comment fetch failed for brand mention %s", m.id)
            session.rollback()
    return total
