...
from .models import (IntelDirection, IntelMention, IntelMentionDirection,
                     IntelStory, IntelFeedLayout, IntelProbe, IntelAlert,
                     IntelThreadContext, IntelSpam, IntelIncident, IntelStoryPoint)
...
def intel_feed_layout_put(
    ...
    if row is None:
        ...
        session.add(row)
    row.direction_ids = json.dumps(body.get("direction_keys", []))
    row.updated_by = user.id
    row.updated_at = datetime.now(timezone.utc)
    session.commit()
    return {"direction_keys": body.get("direction_keys", []),
