...
def _write_m2m(session, mention, primary_direction_id, terms_by_key, id_by_key) -> None:
    ...
    The primary direction is always written with match_type='source'. Every
    direction whose geo_terms match the text gets match_type='geo'. Dedup on
    (mention_id, direction_id) is enforced by a UNIQUE constraint.
    """
        ...
        if probe.kind == "chat":
            ...
            for post in (posts or []):
                ...
                try:
                    session.add(mention)
                    session.flush()
                    _write_m2m(session, mention, mention.direction_id,
