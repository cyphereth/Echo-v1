...
def test_list_and_detail(monkeypatch, tmp_path):
    ...
    st = Story(brand_id=1, title="кризис", first_seen_at=now, last_seen_at=now, post_count=2)
    s.add(st); s.flush()
    inc = Incident(brand_id=1, story_id=st.id, title="i",
                   first_seen_at=now, last_seen_at=now)
    s.add(inc); s.flush()
    s.add(Mention(brand_id=1, platform="telegram", post_id="p", author="@a",
                  text="x", source="niche", incident_id=inc.id, created_at=now))
    s.add(StoryPoint(story_id=st.id, bucket_start=now,
