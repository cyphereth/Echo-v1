...
export function SourcesScreen({ scope }) {
  const [rows, setRows] = useState([]);
  const [val, setVal] = useState('');
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState(null);

  const isTopic = scope?.kind === 'topic';
  const load = () => {
    if (!isTopic) { setRows([]); return; }
    api.getTopicSources(scope.id).then(setRows).catch(() => setRows([]));
  };
  useEffect(load, [scope?.kind, scope?.id]);

  if (!isTopic) {
    return <div style={{ padding: 24, color: 'var(--fg-3)', fontSize: 14 }}>
      Панель источников доступна в режиме «Новости» по выбранной теме.
    </div>;
  }
  ...
  const add = async () => {
    ...
    try { await api.addTopicSource(scope.id, h); setVal(''); load(); }
    catch (e) { setErr(String(e.message || e).includes('409') ? 'Уже добавлен' : 'Не удалось добавить'); }
    finally { setBusy(false); }
  };
  const del = async (id) => { try { await api.deleteTopicSource(scope.id, id); load(); } catch { /* ignore */ } };
